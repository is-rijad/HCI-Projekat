...
namespace Backend.Servisi
{
    public class AuthServis
    {
        ...
        {
            string? cookie = _httpContextAccesor.HttpContext?.Request.Cookies["auth-token"];
            if (cookie == null)
                return null;
        ...
        public async Task<KorisnickiNalog?> GetNalogInfo()
        {
            ...
            if (token == null)
                return null;
            var record = await _dbContext.Tokeni.Include(t => t.KorisnickiNalog).FirstOrDefaultAsync(t => t.Token == token.Token);
            if (record == null)
                return null;
        ...
        public async Task<Tokeni> UlogujKorisnika(UlogujKorisnikaEndpointReq req)
        {
            ...
            else throw new Exception("Korisnicki podaci nisu ispravni!");
            return tokenObj;
        }
        ...
        private string GenerisiToken()
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                rezultat += znakovi[Random.Shared.Next(znakovi.Length)];
            }
            return rezultat;
        }
    }
